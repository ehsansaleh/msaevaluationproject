...
#This step Calculates the Promals with dali alignment
if [ -z "$1" ] || [ -z "$2" ] || [ -z "$3" ]; then
...
outputdir=../../ProcessedData/PromalsDaliFiles/"$NucleicOrAmino"Acids/$DatasetName
maxjobs=14
...
while [[ $currentjob -lt $alljobs ]]; do
		...
		if [ ! -s ${outputfilesarray[$currentjob]}/$myfasta.faa.promals.aln ]; then
			...
			python /projects/tallis/ehsan/promals/promals_package/bin/promals $myfastaname -dali 1 -tmalign 0 -fast 0 > $myfastaname.NodeLog &
		fi
		cd $oldloc
