...
outputdir=../../ProcessedData/ProbConsAlignments/"$NucleicOrAmino"Acids/$DatasetName
maxjobs=12
...
echo Doing ProbCons Alignment on Raw Fasta Files ...
cd $inputdir
echo $inputdir
inputfilesarray=()
outputfilesarray=()
...
while [[ $currentjob -lt $alljobs ]]; do
		...
		if [[ ! -s ${outputfilesarray[$currentjob]} ]];then
			probcons ${inputfilesarray[$currentjob]} -c 5 -ir 1000 -pre 20 > ${outputfilesarray[$currentjob]} &
		fi
