...
#This step Calculates the POA alignment
if [ -z "$1" ] || [ -z "$2" ]; then
...
outputdir=../../ProcessedData/POAAlignments/"$NucleicOrAmino"Acids/$DatasetName
maxjobs=12
...
echo Doing POA Alignment on Raw Fasta Files ...
cd $inputdir
echo $inputdir
inputfilesarray=()
outputfilesarray=()
...
while [[ $currentjob -lt $alljobs ]]; do
		...
		if [[ ! -s ${outputfilesarray[$currentjob]} ]];then
			{ poa -read_fasta ${inputfilesarray[$currentjob]} -clustal ${outputfilesarray[$currentjob]}.clustal /projects/tallis/ehsan/poa/blosum80.mat; t_coffee -other_pg seq_reformat -in=${outputfilesarray[$currentjob]}.clustal -output fasta_aln > ${outputfilesarray[$currentjob]}; rm ${outputfilesarray[$currentjob]}.clustal; } &
		fi
