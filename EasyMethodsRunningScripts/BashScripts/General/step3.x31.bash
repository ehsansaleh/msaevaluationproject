...
#This step Calculates the Promals3D alignment
if [ -z "$1" ] || [ -z "$2" ] || [ -z "$3" ]; then
...
outputdir=../../ProcessedData/Promals3DFiles/"$NucleicOrAmino"Acids/$DatasetName
maxjobs=14
...
echo Doing Promals3D Alignment on Raw Fasta Files ...
cd $inputdir
echo $inputdir
inputfilesarray=()
outputfilesarray=()
...
while [[ $currentjob -lt $alljobs ]]; do
		...
		if [ ! -s ${outputfilesarray[$currentjob]}/$myfasta.faa.promals.aln ]; then
			...
			python /projects/tallis/ehsan/promals/promals_package/bin/promals $myfastaname -dali 0 -tmalign 1 -fast 1 > $myfastaname.NodeLog &
		fi
		cd $oldloc
